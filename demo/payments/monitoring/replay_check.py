...
# The described healthy flows must be clean.
EXPECTED_HEALTHY_VIOLATIONS = 0
# The fault section's intended violations: pF rule-3 timeout, pG rule-1
# (re-capture of a frozen payment), pH rule-2 (disputed close, no refund).
# The terminal-window seed pW is intentionally NOT caught (0 violations).
EXPECTED_FAULT_VIOLATIONS = 3
...
def main() -> int:
    ...
    fault_violations = _report("FAULTS (must violate)", fault_v, fault_by)

    ok = True
    if healthy_violations != EXPECTED_HEALTHY_VIOLATIONS:
        ok = False
        print(f"\nCONFLICT: {healthy_violations} violation(s) on the described "
              f"HEALTHY flows (expected {EXPECTED_HEALTHY_VIOLATIONS}). Rules 1 and 2 "
              "as written forbid lifecycles the user described. This gate stays RED "
              "until the user resolves it (see SUGGESTED_POLICIES.md).")
    if fault_violations != EXPECTED_FAULT_VIOLATIONS:
        ok = False
        print(f"\nFAULT MISMATCH: {fault_violations} violation(s), "
              f"expected {EXPECTED_FAULT_VIOLATIONS}.")
